 ...
 * @return array
 */
function acme_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'acme_page_menu_args' );
 ...
 * @return array
 */
function acme_body_classes( $classes ) {
...
add_filter( 'body_class', 'acme_body_classes' );


/**
 * Mark Posts/Pages as Untiled when no title is used
 */
add_filter( 'the_title', 'acme_title' );

function acme_title( $title ) {
  if ( $title == '' ) {
    return 'Untitled';
  } else {
    return $title;
...
function custom_password_form() {
  ...
  <div class="row">
    <div class="col-lg-10">
        <p>' . __( "This post is password protected. To view it please enter your password below:" ,'acme') . '</p>
        <label for="' . $label . '">' . __( "Password:" ,'acme') . ' </label>
      <div class="input-group">
        <input class="form-control" value="' . get_search_query() . '" name="post_password" id="' . $label . '" type="password">
        <span class="input-group-btn"><button type="submit" class="btn btn-default" name="submit" id="searchsubmit" vvalue="' . esc_attr__( "Submit",'acme' ) . '">' . __( "Submit" ,'acme') . '</button>
        </span>
      </div>
    </div>
  </div>
 ...
 */
add_filter( 'the_content', 'acme_add_custom_table_class' );
function acme_add_custom_table_class( $content ) {
  return str_replace( '<table>', '<table class="table table-hover">', $content );
}

if ( ! function_exists( 'acme_social_icons' ) ) :
/**
 * Display social links in footer and widgets
 */
function acme_social_icons(){
...
if( !function_exists( 'acme_social' ) ) :
	/**
	 * Fallback function for the deprecated function acme_social
	*/
function acme_social(){
 ...
 */
function acme_footer_links() {
  // display the WordPress Custom Menu if available
  wp_nav_menu(array(
    'container'       => '',                              // remove nav container
    'container_class' => 'footer-links clearfix',   // class of container (should you choose to use it)
    'menu'            => __( 'Footer Links', 'acme' ),   // nav name
    ...
    'fallback_cb'     => 'acme_footer_links_fallback'  // fallback function
  ));
} /* end acme footer link */


/**
 * Get Post Views - for Popular Posts widget
 */
function acme_getPostViews($postID){
...
function acme_setPostViews($postID) {
    $count_key = 'post_views_count';
...
if ( ! function_exists( 'acme_call_for_action' ) ) :
/**
 * Call for action button & text area
 */
function acme_call_for_action() {
...
if ( ! function_exists( 'acme_featured_slider' ) ) :
/**
 * Featured image slider
 */
function acme_featured_slider() {
    if ( is_front_page() && of_get_option('acme_slider_checkbox') == 1 ) {
      echo '<div class="flexslider">';
        echo '<ul class="slides">';

          $count = of_get_option('acme_slide_number');
          $slidecat = of_get_option('acme_slide_categories');
...
if ( ! function_exists( 'acme_footer_info' ) ) :
/**
 * function to show the footer info, copyright information
 */
function acme_footer_info() {
  global $acme_footer_info;
  printf( __( 'Theme by %1$s Powered by %2$s', 'acme' ) , '<a href="http://colorlib.com/wp/" target="_blank">Colorlib</a>', '<a href="http://wordpress.org/" target="_blank">WordPress</a>');
}
endif;
 ...
 */
if (!function_exists('get_acme_theme_options'))  {
  function get_acme_theme_options(){
...
add_action('wp_head','get_acme_theme_options',10);
